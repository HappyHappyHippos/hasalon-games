...
export const telephoneModule: GameModule = {
  meta: { id: 'telephone', name: 'Broken Telephone', tagline: 'Write it. Draw it. Watch it go wonderfully wrong.', minPlayers: MIN_PLAYERS, maxPlayers: MAX_PLAYERS, controls: 'Write a prompt or guess, and draw with a mouse or finger.', rules: ['Write a short prompt, then pass it on unseen.', 'Alternate drawing and guessing using only the previous step.', 'Reveal every chain together when all contributions are complete.', 'Vote Good, Meh, or Bad on every drawing except your own.', 'The highest score after the reveal wins.'], touchSupported: true, droppableSnapshots: true },

...
import { TANK_STAGE_IDS, TANK_STAGES, type TankStageId } from '@mg/shared/tanks';
import { sfx } from '../../audio';
import { useT } from '../../strings';

interface Props {
  value: TankStageId | 'random';
  disabled?: boolean;
  onChange: (value: TankStageId | 'random') => void;
}

export function TankStageCarousel({ value, disabled, onChange }: Props): JSX.Element {
  const t = useT();

  // All choices: 6 stages + random
  const choices: Array<TankStageId | 'random'> = [...TANK_STAGE_IDS, 'random'];
  ...
  const currentLabel = isRandom
    ? t.stageRandom
    : t.tankStageNames[TANK_STAGE_IDS.indexOf(currentChoice as TankStageId)] ?? currentChoice;

  const backdropUrl = isRandom ? '' : TANK_STAGES[currentChoice as TankStageId]?.backdropUrl ?? '';
    ...
    >
      <div className="stage-carousel__header">
        ...
        <span className="stage-carousel__badge">
          {isRandom ? '🎲' : `${currentIndex + 1} / ${TANK_STAGE_IDS.length}`}
        </span>
      </div>
      ...
      <div className="stage-carousel__main">
        ...
        >
          {!isRandom ? (
            <img src={backdropUrl} alt={currentLabel} className="stage-carousel__img" />
          ) : (
